...
public class PostServiceImpl implements PostService {
	...
	public void createPost(Post post, int currentUserId, List<MultipartFile> files) throws IOException {
		...
		// ✅ 디버깅: insert 후 post_id가 잘 들어갔는지 확인
		System.out.println("[PostService] post_id after insert: " + post.getPost_id());

		System.out.println("[PostService] files: " + (files == null ? "null" : files.size() + "개"));
		// ✅ 파일 업로드 + DB 저장
	...
	@Override
	public void updatePost(Post post, int currentUserId, List<MultipartFile> filess) throws IOException {
